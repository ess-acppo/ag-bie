Feature: Species - 1. Authorisation Required
  ...
  Scenario: 1. Email Alert
    ...
    Then  they are redirected to ALA authenication page before being taken to the Profile Alert page
  ...
  Scenario: 2. Completion of Alert Page after passing through the login screen
    ...
    When   and they finalise their request
    Then  they are returned to the page they came from

 Scenario: 3. Record a sighting
    Given  a successful species search and a species
    When   click 'record a sighting'  and they are not logged in
    Then  they are redirected to ALA authenication page before being taken to 'Report a sighting'
